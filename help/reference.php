<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#release"></use>
  </svg>
  <div class="justify">
    <h1>General</h1>
    Pekosoft Reference is a lookup tool with two views. Includes BPM, Notes, Scales and Chords data. Columns can be toggled in standard view.
  </div>
</div>

<div class="feature-row module">
  <svg class="standard-image-help">
    <use href="/icons.svg#controls"></use>
  </svg>
  <div class="justify">
    <h1>Controls <span class="object">module</span></h1>
    Contains reset and column visibility controls.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#reset"></use>
  </svg>
  <div class="justify">
    <h1>RESET <span class="object">button</span></h1>
    Resets session and all settings.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#field"></use>
  </svg>
  <div class="justify">
    <h1>Column <span class="object">buttons</span></h1>
    Toggle columns for the currently selected mode in standard view. Column labels update automatically to match the active mode.
  </div>
</div>

<div class="feature-row module">
  <svg class="standard-image-help">
    <use href="/icons.svg#tool"></use>
  </svg>
  <div class="justify">
    <h1>Tool <span class="object">module</span></h1>
    A scrollable result view that changes by selected mode. Supports standard view and cards view. BPM lists 1 to 300 with writing, parity, half and double values. Notes lists chromatic notes from A0 to C8 with MIDI and frequency data. Scales lists common scale formulas and C examples. Chords lists common chord formulas, semitone stacks and C examples.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#metronome"></use>
  </svg>
  <div class="justify">
    <h1>BPM <span class="object">button</span></h1>
    Selects BPM mode.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#piano"></use>
  </svg>
  <div class="justify">
    <h1>NOTES <span class="object">button</span></h1>
    Selects Notes mode.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#timeline"></use>
  </svg>
  <div class="justify">
    <h1>SCALES <span class="object">button</span></h1>
    Selects Scales mode.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#value"></use>
  </svg>
  <div class="justify">
    <h1>CHORDS <span class="object">button</span></h1>
    Selects Chords mode.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#view_list"></use>
  </svg>
  <div class="justify">
    <h1>VIEW <span class="object">button</span></h1>
    Toggles standard table view and cards view.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#sort_az"></use>
  </svg>
  <div class="justify">
    <h1>SORT <span class="object">button</span></h1>
    Toggles sort direction.
  </div>
</div>
