<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#release"></use>
  </svg>
  <div class="justify">
    <h1>General</h1>
    Pekosoft Metronome is for keeping time. It has a selection of click sounds, extensive playback controls, and features like blink and haptic feedback. 
  </div>
</div>

<div class="feature-row module">
  <svg class="standard-image-help">
    <use href="/icons.svg#tool"></use>
  </svg>
  <div class="justify">
    <h1>Tool <span class="object">module</span></h1>
    Emulates a classical metronome. Base display buttons can show BPM, Italian tempi or time signature circles.
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#field"></use>
  </svg>
  <div class="justify">
    <h1>BPM <span class="object">button</span></h1>
    Toggles base BPM display. <span class="default">Default: off.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#italy"></use>
  </svg>
  <div class="justify">
    <h1>TEMPI <span class="object">button</span></h1>
    Toggles base Italian tempi display. <span class="default">Default: off.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#beat_signature"></use>
  </svg>
  <div class="justify">
    <h1>SIGNATURE <span class="object">button</span></h1>
    Toggles base signature circles display. <span class="default">Default: on.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#beat_mute"></use>
  </svg>
  <div class="justify">
    <h1>Signature circles <span class="object">buttons</span></h1>
    Toggle individual beat audio mutes. Muted beats still appear in Panel and Timeline. <span class="default">Default: all beats audible.</span>
  </div>
</div>

<div class="feature-row module">
  <svg class="standard-image-help">
    <use href="/icons.svg#controls"></use>
  </svg>
  <div class="justify">
    <h1>Controls <span class="object">module</span></h1>
    Buttons, fields, menus, knobs and sliders are collected in the Controls module. Status bar explains the interface and provides feedback.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#field"></use>
  </svg>
  <div class="justify">
    <h1>BPM <span class="object">field</span></h1>
    Sets beats per minute. <span class="default">Default: 120.</span>
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#menu"></use>
  </svg>
  <div class="justify">
    <h1>Beat <span class="object">menu</span></h1>
    Sets tick beat sound to Click, Kick, Hi-hat, Sine, Square, Sawtooth, Triangle or Piano. <span class="default">Default: Click.</span>
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#menu"></use>
  </svg>
  <div class="justify">
    <h1>Sig <span class="object">menu</span></h1>
    Sets time signature to 2/4, 3/4, 4/4, 5/4, 6/8, 7/8 or 9/8. This also changes accent cycle and signature circles. <span class="default">Default: 4/4.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#play"></use>
  </svg>
  <div class="justify">
    <h1>PLAY <span class="object">button</span></h1>
    Toggles play. <span class="default">Default: off.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#stop"></use>
  </svg>
  <div class="justify">
    <h1>STOP <span class="object">button</span></h1>
    Stops playback and clears current session data in Panel and Timeline.
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#sound"></use>
  </svg>
  <div class="justify">
    <h1>SOUND <span class="object">button</span></h1>
    Toggles audio playback of the click sound. <span class="default">Default: on.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#beat_accent"></use>
  </svg>
  <div class="justify">
    <h1>ACCENT <span class="object">button</span></h1>
    Toggles first beat accent in each bar based on selected signature. <span class="default">Default: on.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#blink"></use>
  </svg>
  <div class="justify">
    <h1>BLINK <span class="object">button</span></h1>
    Toggles the blinking Tool module background. A highly visual and practical feature, even far away from the screen. <span class="default">Default: off.</span>
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#triangle_down"></use>
  </svg>
  <div class="justify">
    <h1>HALF <span class="object">button</span></h1>
    Halves the current BPM. <span class="example">Example: current BPM becomes half.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#triangle_up"></use>
  </svg>
  <div class="justify">
    <h1>DOUBLE <span class="object">button</span></h1>
    Doubles the current BPM. <span class="example">Example: current BPM becomes double.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#haptic"></use>
  </svg>
  <div class="justify">
    <h1>HAPTIC <span class="object">button</span></h1>
    Toggles short vibrations on each beat. This feature works on supported mobile devices with haptic hardware (e.g., most smartphones). <span class="default">Default: off.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#reset"></use>
  </svg>
  <div class="justify">
    <h1>RESET <span class="object">button</span></h1>
    Resets session and all settings.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#slider"></use>
  </svg>
  <div class="justify">
    <h1>Tempo <span class="object">slider</span></h1>
    Sets BPM in integers. Rounds to nearest integer when BPM has decimals. Decrease and Increase buttons support hold. Supports keyboard input of arrow up, arrow down, page up, page down, home and end.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#slider"></use>
  </svg>
  <div class="justify">
    <h1>Volume <span class="object">slider</span></h1>
    Sets tick volume from 0 to 100. Decrease and Increase buttons support hold. <span class="default">Default: 100.</span>
  </div>
</div>

<div class="feature-row module">
  <svg class="standard-image-help">
    <use href="/icons.svg#timeline"></use>
  </svg>
  <div class="justify">
    <h1>Timeline <span class="object">module</span></h1>
    Displays tempo visualization over time. X axis is time. Y axis is tempo.
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#guides"></use>
  </svg>
  <div class="justify">
    <h1>GUIDES <span class="object">button</span></h1>
    Toggles guides in Timeline. Horizontal BPM reference lines show labeled values (30, 60, 90, 120, 150, 180 and 210). <span class="default">Default: off.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#copy"></use>
  </svg>
  <div class="justify">
    <h1>COPY <span class="object">button</span></h1>
    Copies the Timeline canvas as a PNG image to clipboard.
  </div>
</div>

<div class="feature-row module">
  <svg class="standard-image-help">
    <use href="/icons.svg#panel"></use>
  </svg>
  <div class="justify">
    <h1>Panel <span class="object">module</span></h1>
    Text-based output showing calculated values. Timestamp, Beat number, BPM and Interval in MS.
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#timeline"></use>
  </svg>
  <div class="justify">
    <h1>BEATS <span class="object">button</span></h1>
    Shows beat history output in Panel.
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#italy"></use>
  </svg>
  <div class="justify">
    <h1>TEMPI <span class="object">button</span></h1>
    Shows Italian tempi list in Panel.
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#wrap_text"></use>
  </svg>
  <div class="justify">
    <h1>WRAP <span class="object">button</span></h1>
    Toggles text wrap in the panel so long lines wrap instead of scrolling sideways. <span class="default">Default: off.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#alpha"></use>
  </svg>
  <div class="justify">
    <h1>COLOR <span class="object">button</span></h1>
    Toggles syntax color in the panel text preview. <span class="default">Default: on.</span>
  </div>
</div>

<div class="feature-row border"><svg class="standard-image-help">
    <use href="/icons.svg#copy"></use>
  </svg>
  <div class="justify">
    <h1>COPY <span class="object">button</span></h1>
    Copies the output from the panel to clipboard.
  </div>
</div>

<div class="feature-row module">
  <svg class="standard-image-help">
    <use href="/icons.svg#meter"></use>
  </svg>
  <div class="justify">
    <h1>Meters <span class="object">module</span></h1>
    Shows shared Pekosoft meter views: spectroscope, level meter, oscilloscope and wavescope.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#guides"></use>
  </svg>
  <div class="justify">
    <h1>GUIDES <span class="object">button</span></h1>
    Toggles meter guides.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#eq"></use>
  </svg>
  <div class="justify">
    <h1>SPECTROSCOPE <span class="object">button</span></h1>
    Shows the spectroscope meter view.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#meter"></use>
  </svg>
  <div class="justify">
    <h1>LEVEL <span class="object">button</span></h1>
    Shows the level meter view.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#wavelength"></use>
  </svg>
  <div class="justify">
    <h1>OSCILLOSCOPE <span class="object">button</span></h1>
    Shows the oscilloscope view.
  </div>
</div>

<div class="feature-row border">
  <svg class="standard-image-help">
    <use href="/icons.svg#pulse"></use>
  </svg>
  <div class="justify">
    <h1>WAVESCOPE <span class="object">button</span></h1>
    Shows the wavescope view.
  </div>
</div>
